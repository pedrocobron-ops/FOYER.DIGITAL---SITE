...
/* A tarja de cookies é presa no fim da tela e passa por cima de tudo. Ela
   cobria exatamente estas duas setas: o leitor de primeira viagem abria a
   revista e não conseguia virar a página. Enquanto a tarja está no ar, as
   setas (e a barra de navegação da revista) sobem a altura dela. */
html.tem-aviso .rv-quina{ bottom:calc(6px + var(--aviso-alt, 0px)); }
html.tem-aviso .rv-nav{ bottom:calc(14px + var(--aviso-alt, 0px)); }
...
<main id="conteudo" class="wrap">
  <style>
    ...
    /* As animações: cada formato se apresenta sozinho, em loop.
       Cada gesto imita o que o formato FAZ na página — a cortina abre, o
       entreato entra pela lateral no meio do texto, o cartaz é pregado na
       parede, a página vira, a meia sobe do pé. E nenhum deles congela no
       meio do caminho: depois de entrar, o selo continua respirando, para a
       fileira nunca parecer uma foto. (Refeito em 31/07/2026: antes o
       movimento durava 1,2s de cada 5s e o resto era pura imobilidade.) */
    ...
    @keyframes miPulso{
      ...
      13%{ opacity:1; transform:scale(1.06); }
    ...
    @keyframes miDesliza{
      ...
      93%,100%{ opacity:0; transform:translateX(26px); } }
    /* o cartaz é PREGADO na parede: sobe de baixo, torto, e assenta */
    .mini-cartaz .selo{ left:50%; top:50%; width:64px; height:64px;
      transform:translate(-50%,-50%); transform-origin:50% 0;
      animation:miCartaz 5s ease-in-out infinite; }
    @keyframes miCartaz{
      0%,4%{ opacity:0; transform:translate(-50%,20px) rotate(-9deg) scale(.86); }
      15%{ opacity:1; transform:translate(-50%,-58%) rotate(4deg) scale(1.04); }
      24%{ transform:translate(-50%,-50%) rotate(-2deg) scale(1); }
      34%{ transform:translate(-50%,-50%) rotate(0deg); }
      60%{ transform:translate(-50%,-52%) rotate(1.5deg); }
      82%{ opacity:1; transform:translate(-50%,-50%) rotate(0deg); }
      93%,100%{ opacity:0; transform:translate(-50%,18px) rotate(7deg) scale(.86); } }
    @keyframes miSobe{
      0%,4%{ transform:translateY(115%); }
      14%{ transform:translateY(-8%); }
      20%{ transform:translateY(0); }
      44%{ transform:translateY(-13%); }
      64%{ transform:translateY(-3%); }
      82%{ transform:translateY(0); }
      93%,100%{ transform:translateY(115%); } }
    ...
    @keyframes miVira{
      0%,4%{ opacity:0; transform:rotateY(78deg); }
      16%{ opacity:1; transform:rotateY(-6deg); }
      26%{ transform:rotateY(0deg); }
      44%{ transform:rotateY(11deg); }
      64%{ transform:rotateY(-4deg); }
      82%{ opacity:1; transform:rotateY(0deg); }
      93%,100%{ opacity:0; transform:rotateY(78deg); } }
    ...
    /* ESTA PÁGINA ANIMA PARA TODO MUNDO (ordem do Pedro, 31/07/2026).
       O anuncie.html é a vitrine comercial da casa: é aqui que o produtor
       decide se o FOYER parece um veículo vivo ou um panfleto. O Pedro pediu
       a cortina de volta e, depois, os cartões — então a regra passa a ser
       uma só, e não um remendo: nesta página nada é desligado por causa da
       preferência de movimento reduzido. O RESTO DO SITE continua obedecendo
       (matéria, capa, revista, cadastro), porque lá o leitor está lendo, e
       ali a preferência dele manda. */
...
(function(){
  ...
  var maxVisto = 1;
  var azMexeMenos = false;   // nesta página o orçamento sobe contando, para todo mundo
